...
public class TestRecognizer {
   ...
   public void setup(){
    ...
    public void testResample(){
        int n = 10;
        Deque<Point> resampled = recognizer.resamplePoints(originalPoints, n);
        ...
        resampled = recognizer.resamplePoints(circleTemplate, 64);
   ...
   public void testRotateBy(){
    ...
    /**
    * Tests that squarePoints works
    * SquarePoints 
    */
   @Test
   public void testSquarePoints(){
       Deque<Point> box = new ArrayDeque<>(4);
       box.add(new Point(-50,50));
       box.add(new Point(50, 50));
       box.add(new Point(50,-50));
       box.add(new Point(-50, -50));
       Deque<Point> scaled = recognizer.squarePoints(box);
       ...
       while (itScaled.hasNext()){
           ...
           assertEquals(boxPoint.scale(2.5), scaledPoint);
       }
   }
   ...
   public void testPathDistance() {
    ...
    IOManager ioManager = new IOManager();
    Deque<Point> templateGesture = recognizer.resamplePoints(ioManager.loadGesture("arrowTemplate.xml"), 64);
    Deque<Point> testGesture = recognizer.resamplePoints(ioManager.loadGesture("arrowTest.xml"), 64);
    ...
    /**
     * Tests if boundingBox() sets width and height without modifying the size of the path
     */
    @Test
    public void testBoundingBox() {
        recognizer.boundingBox(originalPoints);
        double[] dimensions = recognizer.getDimensions();
        assertEquals(ORIGINAL_N, originalPoints.size());
        assertEquals(19, dimensions[0]);
        assertEquals(0, dimensions[1]);
    }

   /**
    * tests if a template is added and if it is modified by resamplePoints, rotatePoints, and squarePoints
    */
    @Test
    public void testAddTemplate() {
        IOManager ioManager = new IOManager();
        Deque<Point> circleTemplate = ioManager.loadGesture("circleTemplate.xml");
        assertTrue(recognizer.templatesIsEmpty());
        recognizer.addTemplate("circleTemplate", circleTemplate);
        assertFalse(recognizer.templatesIsEmpty());
        assertNotEquals(circleTemplate, recognizer.getTemplates().get(0).getPath());
    }


    /**
     * Tests if getCentroid() works without modifying the size of the path
     */
    @Test
    public void testCentroid() {
        IOManager ioManager = new IOManager();
        Deque<Point> circleTemplate = ioManager.loadGesture("circleTemplate.xml");
        int size = circleTemplate.size();
        Point centroid = recognizer.getCentroid(circleTemplate);
        assertEquals(size, circleTemplate.size());
        assertEquals(303.180, centroid.getX(),0.001);
        assertEquals(309.127, centroid.getY(), 0.001);
    }
   ...
   public void testRecognize(){
       ...
       Deque<Point> circleTemplate = ioManager.loadGesture("circleTemplate.xml");
       recognizer.addTemplate("arrowTemplate", templateGesture);
       recognizer.addTemplate("circleTemplate", circleTemplate);

       Deque<Point> testGesture = ioManager.loadGesture("arrowTest.xml");
       double score = recognizer.recognize(testGesture).getScore();
       ...
       recognizer.addTemplate("arrowTest", testGesture);
       score = recognizer.recognize(testGesture).getScore();
